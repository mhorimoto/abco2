...
void UECS_EEPROM_SaveCCMAttribute(int ccmid) {
  ...
  if(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_L_CCM_TOTAL>EEPROM_CCMEND){return;}//out of memory
#endif
  if(EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ROOM)
     !=(unsigned char)(U_ccmList[ccmid].baseAttribute[AT_ROOM])) {
    EEPROM.write(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ROOM,
		 (unsigned char)(U_ccmList[ccmid].baseAttribute[AT_ROOM]));
  }
  if(EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_REGI)
     !=(unsigned char)(U_ccmList[ccmid].baseAttribute[AT_REGI])) {
    EEPROM.write(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_REGI,
		 (unsigned char)(U_ccmList[ccmid].baseAttribute[AT_REGI]));
  }
  if(EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ORDE_L)
     !=(unsigned char)(U_ccmList[ccmid].baseAttribute[AT_ORDE])) {
    EEPROM.write(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ORDE_L,
		 (unsigned char)(U_ccmList[ccmid].baseAttribute[AT_ORDE]));
  }
  if(EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ORDE_H)
     !=(unsigned char)(U_ccmList[ccmid].baseAttribute[AT_ORDE]/256)) {
    EEPROM.write(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ORDE_H
		 ,(unsigned char)(U_ccmList[ccmid].baseAttribute[AT_ORDE]/256));
  }
  if(EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_PRIO)
     !=(unsigned char)(U_ccmList[ccmid].baseAttribute[AT_PRIO])) {
    EEPROM.write(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_PRIO,
		 (unsigned char)(U_ccmList[ccmid].baseAttribute[AT_PRIO]));
  }
}
...
void UECS_EEPROM_LoadCCMSetting(int ccmid) {
  ...
  U_ccmList[ccmid].baseAttribute[AT_ROOM] =
    EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_ROOM) & 127;
  U_ccmList[ccmid].baseAttribute[AT_REGI] =
    EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_REGI) & 127;
  ...
  U_ccmList[ccmid].baseAttribute[AT_PRIO]=
    EEPROM.read(ccmid*EEPROM_L_CCM_TOTAL+EEPROM_CCMTOP+EEPROM_L_CCM_PRIO) & 31;
...
//---------------------------------------------------------
void IPv4toByteArray(uint32_t ip,byte *buf) {
  int t;
  byte *o;
  o = buf;
  t = ip & 0xff;
...
*/
void UECSsetCCM(boolean _sender, signed char _num, const char* _name,
		const char* _type,const char* _unit, unsigned short _priority,
		unsigned char _decimal, char _ccmLevel) {
...
void HTTPFilterToBuffer(void) {
  ...
  //decode after %
  for(i=1;i<s_size;i++) {
    if((unsigned char)UECSbuffer[i]<ASCIICODE_SPACE ||
       (UECSbuffer[i-1]=='&' && UECSbuffer[i]=='S' && UECSbuffer[i+1]=='=')) {
      UECSbuffer[i]='\0';
      break;
    }
